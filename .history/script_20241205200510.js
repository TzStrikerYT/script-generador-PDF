function generateCertificates() {
  ...
  function drawFormattedText(doc, textParts, x, y, options = {}) {
    const maxWidth = options.maxWidth || 170; // Maximum text width
    const lineHeight = options.lineHeight || 5; // Line height
    let currentY = y; // Current vertical position
    let currentLine = ""; // Accumulated text for current line
    let currentLineParts = []; // Accumulated text parts for current line
    ...
    const drawLine = () => {
      ...
      // Handle justification for lines with multiple words
      const words = currentLine.trim().split(/\s+/);
      const spaceCount = words.length - 1;
  
      // Draw each part of the line respecting styles
      if (spaceCount > 0 && availableSpaceWidth > 0) {
        ...
        // For single word lines or when justification isn't possible, left-align
        currentLineParts.forEach((part) => {
          const { text, bold } = part;
      ...
      currentLineParts = []; // Reset line parts
    };
    ...
    textParts.forEach((part) => {
      ...
      words.forEach((word, i) => {
        const space = i < words.length - 1 ? " " : ""; // Add space between words
        const testLine = currentLine + word + space;
        const testWidth = doc.getTextWidth(testLine);
  
        if (testWidth > maxWidth) {
          // If line exceeds width, draw current line
          drawLine();
        }
